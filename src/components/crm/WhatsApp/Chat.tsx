  ...
  const { sendMedia, isSendingMedia } = useWhatsAppMedia()
	...
	return (
		<div className="flex flex-col h-full">
			{/* Header */}
			<div className="h-14 border-b border-border px-4 py-2 flex items-center gap-3 bg-[#161616] text-[#E5E7EB]">
				...
				<div className="flex-1 min-w-0">
					...
					{lead && (
						<div className="mt-1 flex items-center gap-1">
							<Badge variant="secondary" className="text-[10px] uppercase bg-[#1a1a1a] text-[#A1A1AA] border-0">{lead.status}</Badge>
						</div>
					)}
				</div>
			</div>
					...
					<div key={label}>
						...
						{items.map((m) => (
							<div key={m.id} className={`w-full flex ${m.direction === 'outbound' ? 'justify-end' : 'justify-start'}`}>
								<div className={`max-w-[80%] px-3 py-2 text-[13px] shadow ${m.direction === 'outbound' ? 'bg-[#2B2B2B] text-[#E5E7EB] rounded-2xl rounded-br-sm' : 'bg-[#1F1F1F] text-[#E5E7EB] rounded-2xl rounded-bl-sm'}`}>
									<div className="whitespace-pre-wrap break-words leading-relaxed">{m.content}</div>
									<div className={`mt-1 text-[10px] flex items-center gap-1 justify-end text-[#9CA3AF]`}>
										<span>{new Date(m.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
										{m.direction === 'outbound' && (
											<span className="ml-1 flex items-center gap-0.5">
												{m.status === 'sent' && <Check className="h-3 w-3" />}
												{m.status === 'delivered' && <CheckCheck className="h-3 w-3" />}
												{m.status === 'read' && <CheckCheck className="h-3 w-3 text-[#60A5FA]" />}
												{m.status === 'failed' && (
			...
			{/* Composer */}
			<div className="h-16 border-t border-border px-3 flex items-center gap-2 bg-[#1A1A1A] dark:bg-[#141414]">
				<Popover open={quickOpen} onOpenChange={setQuickOpen}>
					<PopoverTrigger asChild>
						<Button title="Mensagens rápidas" variant="ghost" size="icon" className="h-10 w-10 rounded-full text-[#54656F] hover:bg-black/5 dark:text-[#8696A0]">
				...
				<Button title="Anexar arquivo" variant="ghost" size="icon" className="h-10 w-10 rounded-full text-[#54656F] hover:bg-black/5 dark:text-[#8696A0]" onClick={() => fileInputRef.current?.click()} disabled={!lead || isSendingMedia}>
				...
				<Input
					...
					className="rounded-full bg-[#1f1f1f] text-[#E5E7EB] placeholder:text-[#9CA3AF] border-0"
					onKeyDown={(e) => {
